    ...
    match fd_budget::try_set_fd_limit(DESIRED_DAEMON_SOFT_FD_LIMIT) {
        Ok(limit) => {
            if limit < MINIMUM_DAEMON_SOFT_FD_LIMIT {
                ...
                println!("The kaspad node requires a setting of at least {MINIMUM_DAEMON_SOFT_FD_LIMIT} to operate properly.");
                println!("Please use the following command to try increasing the limit up to {DESIRED_DAEMON_SOFT_FD_LIMIT}:");
